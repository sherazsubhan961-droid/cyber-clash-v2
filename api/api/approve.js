...
export default async function handler(req, res) {
    ...
    if (action === 'reject') {
        return res.status(200).send(`<div style="background:#0f172a;color:#fff;font-family:sans-serif;text-align:center;padding:50px;height:100vh;margin:0;display:flex;flex-direction:column;justify-content:center;"><h1 style="color:#F43F5E;">🚨 Registration Rejected</h1></div>`);
    }

    if (action === 'approve') {
        if (!token) return res.status(400).send("Token identifier tracking missing.");
        ...
        try {
            ...
            return res.status(200).send(`<div style="background:#0f172a;color:#fff;font-family:sans-serif;text-align:center;padding:50px;height:100vh;margin:0;display:flex;flex-direction:column;justify-content:center;"><h1 style="color:#10B981;">✅ Tickets Dispatched Successfully!</h1><p style="color:#94a3b8;">Competitors have been logged on the active entry charts.</p></div>`);

        } catch (err) {
            return res.status(500).send("Error decoding tracking data.");
        }
    }
}
